...
## 精度軸（H3）＋ MPS 自己非再現（2026-07-07）

MPS・同seed・strict の **run-to-run** 予測不一致率を精度別に測定（shakespeare, 814k）。
...
- **発見1: MPS は自己再現不能**。CPU/CUDA は run間 bit-identical だが、**MPS は同ハード同seedでも run-to-run で非決定増幅（fp32 で 3200步 15.4%）**。→ 階層精密化「CPU・CUDA=ローカル決定性あり / MPS=自己再現すら不能」。
- **発見2: 低精度が onset を早める（H3 支持）**。400步で fp32 6.2% vs bf16 15.2% / fp16 10.2%。実務標準の bf16/fp16 が分岐を早く・大きくする。
- caveat: 単一 seed ペア（不一致率に揺らぎ、fp32 が 17→15.4 と非単調）。robust 化に複数 seed ペア要。傾向は明瞭。
